...
module Opts = struct
  ...
  let default_options = {
    ...
    esproposal_optional_chain = Options.ESPROPOSAL_WARN;
    facebook_fbt = None;
...
let parse_options config lines =
  ...
  let options = parse config.options lines
    ...
    |> define_opt "esproposal.optional_chain" {
      initializer_ = USE_DEFAULT;
      flags = [];
      optparser = optparse_esproposal_feature_flag ~allow_enable:true;
      setter = (fun opts v -> Ok {
        opts with esproposal_optional_chain = v;
      });
    }
...
let esproposal_optional_chain c = c.options.Opts.esproposal_optional_chain
